...
public class NewsServiceIntTest {
    private final static String BASE_URL = "http://localhost:8080/studi-app/api";
    private final static String CREATE_NEWS = "createNews";
    private final static String GET_ALL_NEWS = "getAllNews";
    private final static String UPDATE_SINGLE_NEWS = "updateNews";
    ...
    public void testPost() throws IOException {
        final Optional<Link> createPersonLink = callDispatcherAndGetHeaderLinkWithRelType(CREATE_NEWS);
        ...
        System.out.println(this.genson.serialize(news));

        final RequestBody body = RequestBody.create(
                MediaType.parse(theCreateLink.getMediaType()),
                this.genson.serialize(news));
    ...
    public void testGetCollection() {
        final Optional<Link> getAllPersons = callDispatcherAndGetHeaderLinkWithRelType(GET_ALL_NEWS);
    ...
    public void testPut() throws Exception {
        ...
        final Link theCreateLink = createPersonLink.get();
//        File file = new File("src/test/RESOURCES/035a3e0a1b52c66ddbbbd50bb4bc19a2.gif");
        final News news = News.builder()
                .text("Hello ")
                .title("Welcome")
                .userName("rkrumova97")
							  .time( LocalDateTime.now() )
							  .image( "ZXhhbXBsZSBpbWFnZQ==" )
                .build();
        ...
        final News personResponse =
                this.genson.deserialize(responseGetSingle.body().string(), News.class);
        assertEquals("Welcome", personResponse.getTitle());

        /* Update this person */
        final Optional<Link> linkToUpdate = getResponseHeaderLink(responseGetSingle, UPDATE_SINGLE_NEWS);
        assertTrue(String.format("No link of relType '%s' found.", UPDATE_SINGLE_NEWS),
                linkToUpdate.isPresent());

        personResponse.setTitle("Julius");
